-- Remove study_method from profiles entirely.
-- Order matters: update the trigger first, THEN drop the column,
-- so that next signup doesn't fire a trigger referring to a missing column.
...
ALTER TABLE profiles DROP COLUMN IF EXISTS study_method;
